...
subject_name = 'ad_conversion-value'
...
consumer.subscribe(['ad_conversion'])
...
def send_to_cassandra(message):
    ...
    insert_stmt = session.prepare("""INSERT INTO achievers.conversion_event (
    ad_id, user_id, conversion_timestamp, product_id, revenue, campaign,
    product, target_start_date, target_end_date, age, gender, interests)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""")

    revenue_rounded = round(Decimal(message.get("revenue")), 2)
    ...
    try:
        ...
        cassandra_data = (
                message.get("ad_id"),
                message.get("user_id"),
                message.get("conversion_timestamp"),
